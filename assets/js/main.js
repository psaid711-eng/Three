...
const daggerTemplate = new THREE.Group();

const bladeGeometry = new THREE.CylinderGeometry(0.02, 0.08, 0.6, 8);
...
for (let i = 0; i < NUM_SPHERES; i++) {
	...
	const dagger = daggerTemplate.clone(true);
...
function updateSpheres(deltaTime) {
	...
	for (const sphere of spheres) {
		...
		if (sphere.velocity.lengthSq() > 0.001) {

			const target = sphere.collider.center
				.clone()
				.add(sphere.velocity);

			sphere.mesh.lookAt(target);
